...
def process_data(df_operaciones):
    ...
    etapa_to_alias = df_operaciones.set_index('IDOperacion')['Alias'].to_dict()
    filtered_df['IDOperacion'] = filtered_df['IDOperacion'].astype(str)
    filtered_df['IDOperacion_Alias'] = filtered_df['IDOperacion'].map(lambda x: f"{x} ({etapa_to_alias.get(x, '')})")

    # Obtener valores únicos y ordenarlos alfabéticamente
    unique_etapas_alias = sorted(filtered_df['IDOperacion_Alias'].unique())

    # Selectbox para filtrar por IDEtapa
    selected_etapa_alias = st.selectbox('Select IDOperacion to filter', unique_etapas_alias)
    selected_etapa = selected_etapa_alias.split(' ')[0]
    filtered_result_df = filtered_df[filtered_df['IDOperacion'] == selected_etapa]


    # Realizar cálculos
    result_df = filtered_result_df.groupby(['IDOperacion', 'Ano'])['Monto'].sum().reset_index()
    result_df['Monto Acumulado'] = result_df.groupby('IDOperacion')['Monto'].cumsum().round(2).reset_index(drop=True)

    # Sumar el Porcentaje por IDEtapa y Año
    result_df = result_df.merge(
        filtered_result_df.groupby(['IDOperacion', 'Ano'])['Porcentaje'].sum().reset_index(),
        on=['IDOperacion', 'Ano'],
        how='left'
    )

    # Calcular el Porcentaje Acumulado por IDEtapa
    result_df['Porcentaje Acumulado'] = result_df.groupby('IDOperacion')['Porcentaje'].cumsum().round(2).reset_index(drop=True)
    ...
    result_df_ano_efectiva = filtered_result_df.groupby(['IDOperacion', 'Ano_FechaEfectiva'])['Monto'].sum().reset_index()
    result_df_ano_efectiva['Monto Acumulado'] = result_df_ano_efectiva.groupby('IDOperacion')['Monto'].cumsum().round(2).reset_index(drop=True)

    # Sumar el Porcentaje por IDEtapa y Año de Fecha Efectiva
    result_df_ano_efectiva = result_df_ano_efectiva.merge(
        filtered_result_df.groupby(['IDOperacion', 'Ano_FechaEfectiva'])['Porcentaje'].sum().reset_index(),
        on=['IDOperacion', 'Ano_FechaEfectiva'],
        how='left'
    )

    # Calcular el Porcentaje Acumulado por IDEtapa
    result_df_ano_efectiva['Porcentaje Acumulado'] = result_df_ano_efectiva.groupby('IDOperacion')['Porcentaje'].cumsum().round(2).reset_index(drop=True)
